...
if cred_info.get('age_days', 0) > 25:  # Refresh before 30 days
    print("  Token is getting old, consider refreshing")
    sdk.login(force_login=True)
```
...
class PCloudManager:
    ...
    def list_accounts(self):
        ...
        for name, sdk in self.accounts.items():
            ...
            current = "=I" if name == self.current_account else "  "
...
try:
    ...
    if "invalid" in str(e).lower():
        print("L Invalid email or password")
...
try:
    ...
    if "connect" in str(e).lower():
        print("L Connection failed to EU servers")
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "rate" in str(e).lower() and attempt < max_retries - 1:
                ...
                print(f"ó Rate limited, waiting {wait_time}s...")
...
def diagnose_token_issues():
    ...
    if not cred_info.get('email'):
        print("L No saved credentials found")
        return "no_credentials"
    
    age_days = cred_info.get('age_days', 0)
    if age_days > 30:
        print(f"  Credentials are {age_days:.1f} days old")
        print(">ù Clearing old credentials...")
        sdk.clear_saved_credentials()
        return "expired"
...
def test_authentication():
    ...
    print(">ê Testing pCloud Authentication...")
    ...
    try:
        ...
        print(f"L Credential persistence failed: {e}")
        return False
